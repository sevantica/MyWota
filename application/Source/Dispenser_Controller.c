...
static bool dispenser_has_balance(void)
{
    ...
    bool has_balance = (user_data->balance_ml > 0);
    DISPENSER_DEBUG("has_balance: balance=%lu ml, result=%s", user_data->balance_ml, has_balance ? "YES" : "NO");
    return has_balance;
}
...
static void dispenser_start_dispense_internal(bool no_card_mode, uint32_t target_ml)
{
    if (!no_card_mode) {
        MIFARE_UserData_t *user_data = MIFARE_GetUserData();
        if (!user_data || user_data->balance_ml == 0) {
    ...
    if (no_card_mode) {
        ...
        DISPENSER_CRITICAL("[✓] Dispense STARTED - valve OPEN (balance: %lu ml)", 
                         user_data ? user_data->balance_ml : 0);
    }
}
...
static bool dispense(uint32_t elapsed_ms)
{
    ...
    if (time_since_flow >= DISPENSER_FLOW_WATCHDOG_TIMEOUT_MS) {
        ...
        // Only force removal if in normal mode
        if (!g_no_card_mode) {
    ...
    if (to_deduct > user_data->balance_ml) {
        to_deduct = user_data->balance_ml;  // Don't go negative
    }
    ...
    if (to_deduct > 0) {
        ...
        updated_user_data.balance_ml -= to_deduct;
        ...
        if (since_last_write >= DISPENSER_CARD_WRITE_INTERVAL_MS && g_pending_deduction_ml > 0) {
            ...
            if (result == MIFARE_RESULT_OK) {
                ...
                DISPENSER_DEBUG("Card write: deducted %lu ml total, remaining: %lu ml", 
                             g_pending_deduction_ml, written_user_data ? written_user_data->balance_ml : 0);
#endif
                g_pending_deduction_ml = 0;
    ...
    if (final_user_data && final_user_data->balance_ml == 0) {
...
DispenserResult_t MIFARE_Dispenser_GetStatus(DispenserStatus_t *status)
{
    ...
    if (g_no_card_mode) {
        ...
        MIFARE_UserData_t *user_data = MIFARE_GetUserData();
        status->balance_ml = user_data ? user_data->balance_ml : 0;

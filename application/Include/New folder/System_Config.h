...
/**
 * @brief Loyalty configuration (for both car wash and water dispenser)
 * @note Water dispenser uses volume-based loyalty:
 *   - loyalty_threshold = liters purchased to earn reward (e.g., 100)
 *   - loyalty_reward = free liters earned (e.g., 20)
 */
typedef struct {
    uint32_t wash_duration_seconds;         /* Not used in water dispenser */
    uint32_t card_removal_delay_ms;         /* Not used in water dispenser */
    bool loyalty_enabled;                   /* Enable loyalty program (default false for water) */
    uint32_t loyalty_threshold;             /* Threshold to earn reward: ml (e.g., 100000 for 100L) */
    uint32_t loyalty_reward;                /* Reward amount: ml (e.g., 20000 for 20L) */
} CarWash_Config_t;
...
typedef struct {
    ...
    CarWash_Config_t carwash;               /* Car wash / loyalty config (disabled by default) */
